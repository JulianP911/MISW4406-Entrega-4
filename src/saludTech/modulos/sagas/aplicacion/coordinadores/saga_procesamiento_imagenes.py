import uuid
from anonimizador.modulos.anonimizador.aplicacion.handlers import (
    HandlerAnonimizadorIntegracion,
)
...
from saludTech.seedwork.aplicacion.sagas import (
    CoordinadorOrquestacion,
    Transaccion,
    Inicio,
    Fin,
)
...
from saludTech.modulos.sagas.aplicacion.comandos.anonimizador import (
    AnonimizarImagenMedica,
    DeshacerAnonimizacionImagenMedica,
)
from saludTech.modulos.sagas.aplicacion.comandos.gestor_archivos import (
    EliminarImagenMedica,
)

from saludTech.modulos.gestor_archivos.infraestructura.schemas.v1.comandos import (
    CargarImagenMedica,
    CargarImagenMedicaPayload,
)

from saludTech.modulos.gestor_archivos.dominio.comandos import ComandoCargarImagenMedica
from saludTech.modulos.sagas.aplicacion.comandos.modelosIA import (
    GenerarDataframeImagenMedica,
    EliminarDataframeImagenMedica,
)
from saludTech.modulos.sagas.aplicacion.comandos.validador_anonimizador import (
    ValidarAnonimizacionImagenMedica,
    DeshacerValidacionAnonimizacionImagenMedica,
    ValidarAnonimizacionImagenMedicaPayload,
)
from saludTech.modulos.gestor_archivos.aplicacion.comandos.crear_imagen_medica import (
    CrearImagenMedica,
)
from saludTech.modulos.gestor_archivos.dominio.eventos import (
    ArchivoPublicado as ArchivoPublicadoGestorArchivos,
    ArchivoPublicacionFallida as ArchivoPublicacionFallidaGestorArchivos,
    ArchivoPublicacionRevertida as ArchivoPublicacionRevertidaGestorArchivos,
)
from saludTech.modulos.sagas.dominio.eventos.anonimizador import (
    ImagenAnonimizada,
    ImagenAnonimizacionFallida,
    ImagenAnonimizacionRevertida,
)
from saludTech.modulos.sagas.dominio.eventos.modelosIA import (
    ImagenAnonimizadaValidada,
    ImagenAnonimizadaValidacionFallida,
    ImagenAnonimizadaValidacionRevertida,
)
from saludTech.modulos.sagas.dominio.eventos.validador_anonimizador import (
    ArchivoPublicado,
    ArchivoPublicacionFallida,
    ArchivoPublicacionRevertida,
)
from pulsar.schema import Record, AvroSchema
from pydispatch import dispatcher
import pulsar
from validador_anonimizador.modulos.validador_anonimizador.aplicacion.handlers import (
    HandlerImagenMedicaIntegracion,
)
...
class CoordinadorProcesamientoImagenes(CoordinadorOrquestacion):
    def __init__(self):
        super().__init__()
        from saludTech.modulos.sagas.infraestructura.repositorios import (
            SagaLogSQLRepositorio,
        )
    ...
    def inicializar_pasos(self):
        self.pasos = [
            ...
            Transaccion(
                index=1,
                comando=CargarImagenMedica,
                evento=ArchivoPublicado,
                error=ArchivoPublicacionFallidaGestorArchivos,
                compensacion=EliminarImagenMedica,
            ),
            Transaccion(
                index=2,
                comando=ComandoAnonimizarImagen,
                evento=ImagenAnonimizada,
                error=ImagenAnonimizacionFallida,
                compensacion=DeshacerAnonimizacionImagenMedica,
            ),
            Transaccion(
                index=3,
                comando=ValidarAnonimizacionImagenMedica,
                evento=ImagenAnonimizadaValidada,
                error=ImagenAnonimizadaValidacionFallida,
                compensacion=DeshacerValidacionAnonimizacionImagenMedica,
            ),
            Fin(index=4),
        ]
    ...
    def construir_comando(self, evento: EventoDominio, tipo_comando: type, mensaje):
        ...
        print(
            f"Construyendo comando {tipo_comando.__name__} a partir del evento {evento}"
        )
        print("evento.data", isinstance(evento, ImagenAnonimizada))
        if (
            # hasattr(evento, "data")
            # and isinstance(evento.data, ImagenCargadaPayload)
            # and tipo_comando == CargarImagenMedica
            tipo_comando == CargarImagenMedica
            and evento.__name__ == "ArchivoPublicado"
        ):
            print("ENTROOOOOOOO")
            payload = CargarImagenMedicaPayload(
                id=mensaje.data.id,
                url=mensaje.data.url,
                id_paciente=mensaje.data.id_paciente,
            )
            print(f"Payload: {payload}")
            return CargarImagenMedica(data=payload)
        elif (
            tipo_comando == ComandoAnonimizarImagen
            and evento.__name__ == "ImagenAnonimizada"
        ):
            ...
            payload = ValidarAnonimizacionImagenMedicaPayload(
                id=str(uuid.uuid4()),
                url=mensaje.data.url,
                token_paciente=mensaje.data.id_paciente,
            )
    ...
    def ejecutar_comando(self, comando):
        ...
        if isinstance(comando, CargarImagenMedica):
            ...
            # Dispatch the next event (e.g., ArchivoPublicado)
            payload = ImagenCargadaPayload(
                id=comando.data.id_paciente, url=comando.data.url
            )
            dispatcher.send(
                signal=f"{ArchivoPublicado.__name__}Integracion",
                evento=ArchivoPublicado(
                    id=str(uuid.uuid4()),
                    id_paciente=comando.data.id_paciente,
                    url=comando.data.url,
                ),
            ...
            dispatcher.send(
                ...
                evento=ImagenAnonimizada(id=comando.data.id, url=comando.data.url),
                comando=comando.data,
            )
            ...
            dispatcher.send(
                ...
                evento=ImagenAnonimizadaValidada(
                    id=comando.data.id, url=comando.data.url
                ),
                comando=comando.data,
            )
...
def oir_mensaje(mensaje):
    ...
    try:
        ...
        if isinstance(mensaje, ArchivoPublicado):

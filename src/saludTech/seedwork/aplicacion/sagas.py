...
class CoordinadorOrquestacion(CoordinadorSaga, ABC):
    ...
    def obtener_paso_dado_un_evento(self, evento: EventoDominio):
        ...
        for i, paso in enumerate(self.pasos):
            if not isinstance(paso, Transaccion):
                continue
            print(f"TYPE_EVENTO -----> {type(evento)}")
            print(f"TYPE_PASO -----> {(paso.evento)}")
        ...
        for i, paso in enumerate(self.pasos):
            if not isinstance(paso, Transaccion):
                continue
            print(f"TYPE_EVENTO -----> {type(evento)}")
            print(f"TYPE_PASO -----> {(paso.comando)}")
            if isinstance(evento, paso.comando) or isinstance(evento, paso.error):
                return paso, i
    ...
    def procesar_evento(self, evento: EventoDominio):
        ...
        if self.es_primera_transaccion(index) and not isinstance(evento, paso.error):
            self.iniciar(evento)
            ...
            print(paso.evento)
            self.publicar_comando(evento, self.pasos[index + 1].compensacion)
        elif self.es_ultima_transaccion(index) and not isinstance(evento, paso.error):
            self.terminar()
